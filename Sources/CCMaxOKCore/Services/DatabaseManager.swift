...
public final class DatabaseManager: @unchecked Sendable {
    ...
    nonisolated(unsafe) private static let rateLimitSnapshots = Table("rate_limit_snapshots")
    nonisolated(unsafe) private static let rlId = SQLite.Expression<Int64>("id")
    nonisolated(unsafe) private static let rlTimestamp = SQLite.Expression<Double>("timestamp")
    nonisolated(unsafe) private static let rlFiveHourPct = SQLite.Expression<Double?>("five_hour_used_pct")
    nonisolated(unsafe) private static let rlFiveHourReset = SQLite.Expression<Double?>("five_hour_resets_at")
    nonisolated(unsafe) private static let rlSevenDayPct = SQLite.Expression<Double?>("seven_day_used_pct")
    nonisolated(unsafe) private static let rlSevenDayReset = SQLite.Expression<Double?>("seven_day_resets_at")
    nonisolated(unsafe) private static let rlModel = SQLite.Expression<String?>("model")

    nonisolated(unsafe) private static let dailyUsageTable = Table("daily_usage")
    nonisolated(unsafe) private static let duDate = SQLite.Expression<String>("date")
    nonisolated(unsafe) private static let duSessionCount = SQLite.Expression<Int>("session_count")
    nonisolated(unsafe) private static let duMessageCount = SQLite.Expression<Int>("message_count")
    nonisolated(unsafe) private static let duInputTokens = SQLite.Expression<Int>("total_input_tokens")
    nonisolated(unsafe) private static let duOutputTokens = SQLite.Expression<Int>("total_output_tokens")
    nonisolated(unsafe) private static let duCacheReadTokens = SQLite.Expression<Int>("total_cache_read_tokens")
    nonisolated(unsafe) private static let duCacheCreationTokens = SQLite.Expression<Int>("total_cache_creation_tokens")
    nonisolated(unsafe) private static let duModelsUsed = SQLite.Expression<String?>("models_used")

    nonisolated(unsafe) private static let notificationLog = Table("notification_log")
    nonisolated(unsafe) private static let nlId = SQLite.Expression<Int64>("id")
    nonisolated(unsafe) private static let nlTimestamp = SQLite.Expression<Double>("timestamp")
    nonisolated(unsafe) private static let nlType = SQLite.Expression<String>("type")
    nonisolated(unsafe) private static let nlMessage = SQLite.Expression<String?>("message")
