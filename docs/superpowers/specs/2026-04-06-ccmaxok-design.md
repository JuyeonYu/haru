...
- **최소 지원**: macOS 15 (Sequoia) + Apple Silicon (M1 이상) — Apple Intelligence 활용 대비
...
├── Services/
│   ├── FileAccessManager.swift   -- 파일 접근 추상화 (Sandbox 대응, Security-Scoped Bookmarks)
...
초기 개발은 로컬 빌드로 진행하되, 다음 배포 채널을 염두에 두고 설계한다:

- **App Store**: 샌드박싱 제약 고려 필요. `~/.claude/` 접근을 위해 Security-Scoped Bookmarks + 사용자 명시적 디렉토리 접근 권한 필요. App Sandbox entitlement 포함.
- **Homebrew Cask**: `brew install --cask ccmaxok`로 설치. 샌드박싱 없이 파일 접근 자유로움. DMG 또는 zip 아카이브로 배포.
- **GitHub Releases**: 직접 다운로드용 `.dmg` 제공. 공증(Notarization) 필수.

### 설계 시 고려사항

- App Sandbox에서도 동작하도록 파일 접근을 추상화 (직접 경로 하드코딩 대신 FileAccessManager로 관리)
- 코드 서명(Developer ID) + 공증(Notarization) 파이프라인 준비
- 앱 번들 ID: `com.ccmaxok.app` (가칭)
- Privacy manifest (PrivacyInfo.xcprivacy) 포함 — 파일 접근 사유 명시
